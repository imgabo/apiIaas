...
export class VigilanciacirugiasService {
    ...
    async getVigilancias( id : string) : Promise<VigilanciasCirugiasEntity[]> {
        ...
        const vigilancias = await this.vigilanciasCirugiasRepository.find({where: {paciente : paciente}, relations : ['procedimiento', 'herida', 'paciente', 'usuarioCreacion']})
        return vigilancias;
    }
    ...
    async createVigilancia ( dto : NuevaVigilanciaCirugiaDTO) : Promise<any>{
        ...
        const procedimiento =  await this.procedimientoCirugiasRepository.findOne({where : {id : dto.id_procedimiento}})
        const herida = await this.tipoHeridasRepository.findOne({where : {id : dto.id_herida}})
        const paciente = await this.pacienteRepository.findOne({where: { id : dto.id_paciente }})
        const usuarioCreacion = await this.userRepository.findOne({where : { id : dto.id_usuarioCreacion}})
    ...
    async agregarComentario( dto : nuevoComentarioDTO) {
        ...
        const vigilancia = await this.vigilanciasCirugiasRepository.findOne({where:{ id: dto.id_procedimiento }})
    ...
    async getComentarios ( id : string ) : Promise<ComentariosCirugiasEntity[]>{
        const vigilancia = await this.vigilanciasCirugiasRepository.findOne({where:{id: id}})
        const comentarios = await this.comentarioCirugiaEntity.find({where:{vigilancia : vigilancia}, relations: ['user']})
        return comentarios;
    }
}
