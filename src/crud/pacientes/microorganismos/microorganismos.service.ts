...
export class MicroorganismosService {
  ...
  async finById(id: string): Promise<MicroorganismosEntity> {
    const microorganismo = await this.microorganismoRepository.findOne({
      where: { id: id },
    });
    if (!microorganismo)
      throw new HttpException(
  ...
  async create(dto: NuevoMicroorganismoDTO): Promise<any> {
    const { nombre } = dto;
    const exists = await this.microorganismoRepository.findOne({
      where: { nombre: nombre },
    });
    if (exists)
      throw new HttpException(
  ...
  async update(id: string, dto: NuevoMicroorganismoDTO): Promise<any> {
    ...
    dto.nombre
      ? (microorganismo.nombre = dto.nombre)
      : (microorganismo.nombre = microorganismo.nombre);
