...
export class IarepisService {
    ...
    async finById(id : string): Promise<IarepisEntity>{
        ...
        if (!iarepis)throw new HttpException({
            status: HttpStatus.FORBIDDEN,
            error: 'No Existe',
        }, HttpStatus.FORBIDDEN);
        return iarepis;
    ...
    async create(dto : NuevoIarepisDTO) : Promise<any> {
        const {nombre} = dto;
        const exists = await this.iarepisRepository.findOne({where:{nombre : nombre}});
    ...
    async update(id:string, dto:NuevoIarepisDTO): Promise<any>{
        ...
        dto.nombre ? iarepis.nombre = dto.nombre : iarepis.nombre = iarepis.nombre;
