...
export class ProcedimientosCirugiasService {
    ...
    async findByID(id : string): Promise<procedimientosCirugiasEntity>{
        const procedimiento = await this.procedimientoCirugiaRepository.findOne({where: {id:id}});
        if(!procedimiento)throw new HttpException({
            status: HttpStatus.FORBIDDEN,
            error: 'No existe',
        }, HttpStatus.FORBIDDEN);
    ...
    async create(dto : nuevoProcedimientoCirugiaDTO): Promise<any>{
        const {nombre}= dto;
        const exists = await this.procedimientoCirugiaRepository.findOne({where:{nombre:nombre}});
    ...
    async update(id:string, dto:nuevoProcedimientoCirugiaDTO):Promise<any> {
        ...
        procedimiento.nombre ? procedimiento.nombre = dto.nombre : procedimiento.nombre = procedimiento.nombre;
